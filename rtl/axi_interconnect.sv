module axi_interconnect (
    ...
    // =====================================================
    // Slave 5: CNN Accelerator (0x8000_1000)
    // =====================================================
    ...
    // =====================================================
    logic [5:0] aw_slave_sel;  // Write address slave select
    logic [5:0] ar_slave_sel;  // Read address slave select

    // =====================================================
    // Address decoding - Write Address Channel
    // =====================================================
    always_comb begin
        aw_slave_sel = 6'b000000;
        
        if (m_axi_awvalid) begin
            // Check CNN first (needs 12-bit mask)
            if ((m_axi_awaddr & CNN_MASK) == CNN_BASE) begin
                aw_slave_sel[5] = 1'b1;
            end
    ...
    // =====================================================
    always_comb begin
        ar_slave_sel = 6'b000000;
        
        if (m_axi_arvalid) begin
            // Check CNN first (needs 12-bit mask)
            if ((m_axi_araddr & CNN_MASK) == CNN_BASE) begin
                ar_slave_sel[5] = 1'b1;
            end
            // Check other peripherals (8-bit mask)
